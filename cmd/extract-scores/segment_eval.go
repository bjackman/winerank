...
// printSegmentEval prints a colored evaluation report.
func printSegmentEval(videoID string, result segmentEvalResult) {
	fmt.Println()
	fmt.Println("\x1b[1;36m============================================================\x1b[0m")
	fmt.Printf("\x1b[1;36m  SEGMENTATION EVAL: %s\x1b[0m\n", videoID)
	fmt.Println("\x1b[1;36m============================================================\x1b[0m")

	// Placeholder mapping comparison
	if result.MappingTotal > 0 {
		fmt.Println()
		fmt.Println("\x1b[1mPlaceholder Mappings:\x1b[0m")
		if len(result.MappingErrors) == 0 {
			fmt.Printf("  \x1b[32m✓ All %d mappings correct\x1b[0m\n", result.MappingTotal)
		} else {
			fmt.Printf("  %d/%d correct\n", result.MappingCorrect, result.MappingTotal)
			for _, me := range result.MappingErrors {
				fmt.Printf("  \x1b[31m✗ %s:\x1b[0m\n", me.Placeholder)
				fmt.Printf("      Expected: \x1b[32m%s\x1b[0m\n", me.ExpectedWine)
				fmt.Printf("      Got:      \x1b[31m%s\x1b[0m\n", me.GotWine)
			}
		}
	}
	...
	if len(result.Errors) == 0 {
		fmt.Printf("\x1b[1;32m✓ All %d sentences correctly classified\x1b[0m\n", result.Correct)
	} else {
		fmt.Printf("\x1b[1mSentence Errors (%d):\x1b[0m\n", len(result.Errors))
		for _, e := range result.Errors {
			// Truncate long sentences
			sentText := e.Sentence
			if len(sentText) > 80 {
				sentText = sentText[:77] + "..."
			}
			fmt.Printf("  \x1b[31m[%d]\x1b[0m %s\n", e.SentenceIdx, sentText)
			fmt.Printf("        Expected: \x1b[32m%-20s\x1b[0m  Got: \x1b[31m%s\x1b[0m\n", e.Expected, e.Got)
		}
	}
	...
	if result.TotalSentences > 0 {
		accuracy = float64(result.Correct) / float64(result.TotalSentences) * 100
	}
	color := "\x1b[32m" // green
	if accuracy < 95 {
	...
	fmt.Printf("\x1b[1mSentence accuracy:\x1b[0m %s%d/%d (%.1f%%)\x1b[0m\n",
		color, result.Correct, result.TotalSentences, accuracy)

	if result.MappingTotal > 0 {
		mColor := "\x1b[32m"
		if result.MappingCorrect < result.MappingTotal {
			mColor = "\x1b[31m"
		}
		fmt.Printf("\x1b[1mMapping accuracy:\x1b[0m  %s%d/%d\x1b[0m\n",
			mColor, result.MappingCorrect, result.MappingTotal)
	}
