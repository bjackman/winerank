#!/usr/bin/env python3
"""
vinazion/fetch.py — cache-first fetcher for the vinazion.ch WooCommerce Store API.

Vinazion runs WooCommerce, so this is the same approach as realwines/: each page
of /wp-json/wc/store/v1/products is saved as raw JSON under ./cache/page_N.json
and reused on subsequent runs. Delete cache files (or the whole cache/ dir) to
force a re-fetch.
...
def fetch_all(refresh: bool = False) -> list[dict]:
    """Fetch every page of the catalogue, discovering page count from headers."""
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    ...
    if meta_file.exists() and not refresh:
        ...
        cache_p1.write_text(json.dumps(resp.json(), ensure_ascii=False, indent=2))
        print(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch vinazion.ch catalogue to cache")
    parser.add_argument(
        "--refresh",
        action="store_true",
